...
class vsfs::google {
  ...
  # gtest
  exec { 'download_gmock':
    command   => '/usr/bin/wget http://googlemock.googlecode.com/files/gmock-1.7.0.zip -O gmock-1.7.0.zip',
    cwd       => '/usr/local/src',
  }

  exec { 'unzip_gmock':
    command => '/usr/bin/unzip -o gmock-1.7.0.zip',
    cwd     => '/usr/local/src',
  ...
  exec { 'build_gmock':
    ...
    cwd     => '/usr/local/src/gmock-1.7.0',
    require => [Exec['unzip_gmock'], Class['cpp'], Package['cmake']],
  }

  $content = "export CXXFLAGS='-I/usr/local/src/gmock-1.7.0/include -I/usr/local/src/gmock-1.7.0/gtest/include'
export LDFLAGS='-L/usr/local/src/gmock-1.7.0/gtest -L/usr/local/src/gmock-1.7.0'"
